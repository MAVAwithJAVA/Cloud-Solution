...
function saveDestinations() {
    if (destinationList.length != 0) {
        ...
        xhttp.open("POST", "http://localhost:8081/werknemer_cloud_solution_war_exploded/api/destinationController/addDestinations", true);
...
function getDestinations() {
    if (!isRetrieved) {
        ...
        xhttp.open("GET", "http://localhost:8081/werknemer_cloud_solution_war_exploded/api/destinationController/getDestinations", true);
        xhttp.send();
        isRetrieved = true;
    }
...
function updateDestination() {
    if (validateForm()) {
        let destination = {"destinationId" : document.getElementById("destinationid").value,
            ...
            "location" : document.getElementById("location").value}
        let xhttp = new XMLHttpRequest();
        xhttp.open("PUT", "http://localhost:8081/werknemer_cloud_solution_war_exploded/api/destinationController/putDestination", true);
        xhttp.onreadystatechange = function () {
            if (xhttp.readyState > 3 && xhttp.status == 200) {
...
function removeDestination() {
    ...
    if (destinationId == null || destinationId == "") {
        ...
        let xhttp = new XMLHttpRequest();
        xhttp.open("DELETE", "http://localhost:8081/werknemer_cloud_solution_war_exploded/api/destinationController/deleteDestination", true);
        xhttp.onreadystatechange = function () {
            if (xhttp.readyState > 3 && xhttp.status == 200) {

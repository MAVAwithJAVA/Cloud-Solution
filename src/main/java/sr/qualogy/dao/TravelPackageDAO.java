...
import sr.qualogy.reportresultset.DestinationByVisitFrequency;
import sr.qualogy.reportresultset.PeriodByVisitFrequency;
import sr.qualogy.reportresultset.TravelersAgeByPeriod;
...
//    }
    ...
    public List<TravelersAgeByPeriod> frequencyOfTravelersByAge(int year) {
        ...
        entityManager.getTransaction().begin();
        String jpql1 = "select new sr.qualogy.reportresultset.TravelersAgeByPeriod('1-30'," + year + " , count(t)) from TravelPackage t join t.travelGroup.travelers tgt join t.travelPlan tp where (tgt.age between 1 and 30) and year(tp.startDate) = :year";
        ...
        String jpql2 = "select new sr.qualogy.reportresultset.TravelersAgeByPeriod('31-60'," + year + ", count(t)) from TravelPackage t join t.travelGroup.travelers tgt join t.travelPlan tp where (tgt.age between 31 and 60) and year(tp.startDate) = :year";
        ...
        String jpql3 = "select new sr.qualogy.reportresultset.TravelersAgeByPeriod('61-100'," + year + ", count(t)) from TravelPackage t join t.travelGroup.travelers tgt join t.travelPlan tp where (tgt.age between 61 and 100) and year(tp.startDate) = :year";
...
//        System.out.println(resultList);
    ...
    public List<PeriodByVisitFrequency> frequencyOfTravelersByPeriod(int period) {
        ...
        entityManager.getTransaction().begin();
        String jpql = "select new sr.qualogy.reportresultset.PeriodByVisitFrequency(" + jpqlPeriod + "(t.travelPlan.startDate), count(t))" +
...
//        System.out.println(resultList);
    ...
    public List<DestinationByVisitFrequency> frequencyOfDestinationVisitsByYear(int year) {
        entityManager.getTransaction().begin();
        String jpql = "select new sr.qualogy.reportresultset.DestinationByVisitFrequency(tpts.destination, year(tpts.travelPlan.startDate), count(tpts.destination.destinationId)) " +
